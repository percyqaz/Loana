...
type VerbFile(path: string) =
    ...
    member this.Load() =
        ...
        let read_all_lines() : string seq =
            try File.ReadAllLines(path) |> Seq.where (fun line -> line.Trim() <> "")
        ...
        let handle_line(line: string) =
            ...
            if is_inflection then
                ...
                current_inflections <- current_inflections.Add(VerbInflection.FromString(split_by_equals.[0]), split_by_equals.[1])
            else
                finish_current_verb()
        ...
        let detect_missing() : bool =
            ...
            for requested_tense in verb.Tenses do
                ...
                | VerbTense.Imperative when missing_imperative_inflections() ->
                    Console.WriteLine(sprintf "'%s' is missing imperative inflections" verb.Infinitive.Deutsch, Color.Yellow)
                    has_missing <- true
                | _ -> ()
            has_missing
