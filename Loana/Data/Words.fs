...
module Wordlist =
    ...
    let internal parse_noun_inner (vocab: Vocab, tags: string list) : Noun =
        ...
        while remaining_tags <> [] do
            ...
            | "plural" ->
                if gender.IsNone then
                    failwithf "plural must be set after gender for noun: %O" vocab

                if no_plural then
                    failwithf "plural conflicts with 'no_plural' for noun: %O" vocab
    ...
    let internal parse_verb_inner (vocab: Vocab, tags: string list) : Verb =
        ...
        let mutable past_participle: Knowledge<Vocab> =
            if tags <> [] then KnownNothing else Unknown
    ...
    let parse_vocab: string -> Vocab = parse_core >> fst
