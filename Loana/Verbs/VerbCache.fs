...
type VerbCacheEntry =
    { Verb: Verb; Tense: VerbTense }
    member this.Key = sprintf "verb-%O-%s" this.Tense this.Verb.Infinitive.DeutschAsciiIdentifier
...
type VerbCache(scheduler: ReviewSchedule, words: WordBank) =
    ...
    member inline this.LearningEntries(entries: VerbCacheEntry seq) =
        entries
        |> Seq.where (fun cached_verb -> (this.Scheduler.Get cached_verb.Key).IsNone)

    member inline this.ReviewEntries(entries: VerbCacheEntry seq) =
        entries
        |> Seq.where (fun cached_verb -> (this.Scheduler.Get cached_verb.Key).IsSome)
