...
type VerbInflection =
    ...
    static member FromString(value: string) : VerbInflection =
        if value.StartsWith("pr/") then
            InflectionPerson.FromString(value.Substring(3)) |> Present
        elif value.StartsWith("pa/") then
            InflectionPerson.FromString(value.Substring(3)) |> SimplePast
        elif value.StartsWith("im/") then
            ImperativePerson.FromString(value.Substring(3)) |> Imperative
        else
