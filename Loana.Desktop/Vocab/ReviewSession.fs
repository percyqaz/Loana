...
type ReviewSession(cards: Card array, scheduler: ReviewSchedule, ahead: bool) =
    ...
    override this.Keep (card: Card) : unit = scheduler.Keep(card) |> Seq.iter _.LogTo(this)

    override this.Promote (card: Card) : unit = scheduler.Promote(card) |> Seq.iter _.LogTo(this)
